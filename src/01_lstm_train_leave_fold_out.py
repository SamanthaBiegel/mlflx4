...
# Generate filename
hparams = {"n_epochs": args.n_epochs, "early_stopping": args.early_stopping, "patience": args.patience,
              "hidden_size": args.hidden_size, "num_layers": args.num_layers, "dropout": args.dropout,
              "batch_size": args.batch_size, "learning_rate": args.learning_rate, "scheduler_patience": args.scheduler_patience,
              "scheduler_factor": args.scheduler_factor, "weight_decay": args.weight_decay}
filename = generate_filename("lstm", "lfo", hparams)
...
print(f"LSTM - Mean R2: {np.mean(all_metrics['r2']):.4f} | Mean RMSE: {np.mean(all_metrics['rmse']):.4f} | Mean NMAE: {np.mean(all_metrics['nmae']):.4f} | Mean Abs Bias: {np.mean(all_metrics['abs_bias']):.4f}")
